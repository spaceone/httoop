...
class HTTPServerError(HTTPStatusException):
	u"""SERVER_ERRORS = 5xx
		...
		The server can send the Retry-After header if it knows that the problem is temporary.
	"""
	def to_dict(self):
...
class SWITCHING_PROTOCOLS(object):
	u"""If the client wants to use another protocol (in the Upgrade-header) this is the response
		that the TCP server now speaks another protocol."""
	__metaclass__ = StatusType
	status = 101
	body = None
...
class OK(object):
	u"""The request was successful.
		On GET requests the entity body will be a representation of the requested resource.
		For other methods the entity body contains a representation of the current state of the resource or a description of the performed action"""
	__metaclass__ = StatusType
	status = 200
...
class ACCEPTED(object):
	u"""The request looks valid but will be procecced later. It is an asynchronous action.
		...
		If this is not possible it should give an estimate time when the request will be processed."""
	__metaclass__ = StatusType
	status = 202
...
class NO_CONTENT(object):
	u"""GET: The representation of the resource is empty.
		...
		It is also useful for making series of edits to a single record (a HTML POST form)."""
	__metaclass__ = StatusType
	status = 204
	body = None
...
class PARTIAL_CONTENT(object):
	u"""Partial GET:
		The response does not contains the full representation of a resource only the bytes requested in the Content-Range-header.
		It is often use to resume an interrupted download.
		The Date-header is required, the ETag-header and Content-Location-header are useful."""
	__metaclass__ = StatusType
	status = 206
...
#	status = 207
	...
	#"""This status code indicated that the entity-body contains information about the states of the batch request.
...
class MULTIPLE_CHOICES(object):
	u"""The server has multiple representations of the requested resource.
		And the client e.g. did not specify the Accept-header or the requested representation does not exists.
	"""
	__metaclass__ = StatusType
	status = 300
...
class MOVED_PERMANENTLY(object):
	...
	It can also be send if a resource have moved or renamed to prevent broken links."""
	__metaclass__ = StatusType
	status = 301
...
class SEE_OTHER(object):
	...
	it links to another document which contains a static status message, etc. so the client is not forced to download the data.
...
class NOT_MODIFIED(object):
	...
	The Date-header is required, the ETag-header and Content-Location-header are useful.
	Also the caching headers Expires, Cache-Control and Vary are required if they differ from those sent previously.
	TODO: what to do if the representation format has changed but not the representation itself?
...
class TEMPORARY_REDIRECT(object):
	u"""The request has not processed because the requested resource is located at a different URI.
		...
		for GET this is the same as 303 but for POST, PUT and DELETE it is important that the request was not processed."""
	__metaclass__ = StatusType
	status = 307
...
class UNAUTHORIZED(object):
	u"""The requested resource is protected and no or wrong authentication credentials was given.
		The WWW-Authenticate-header contains information about the accepted authentication method.
		The entity-body should contain information about what was wrong with the given credentials and where to register a new account.
	"""
	__metaclass__ = StatusType
	status = 401
...
class CONFLICT(object):
	u"""If the request would cause to leave the resource in an inconsequent state this status is send.
		Examples: DELETE of a non empty bucket, changing a username to a already taken username.
...
class REQUEST_ENTITY_TOO_LARGE(object):
	u"""The HTTP server can deny too large representations.
		...
		If the server can only not handle the request e.g. because of full disk space it can send the Retry-After-header."""
	__metaclass__ = StatusType
	status = 413
...
class INTERNAL_SERVER_ERROR(object):
	u"""The generic status code. Mostly used when an exception in the request handler occurrs."""
	__metaclass__ = StatusType
	status = 500
