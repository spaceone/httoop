...
def test_inlwithasciifilename(content_disposition):
    h = content_disposition(b'Content-Disposition: inline; filename="foo.html"')
    assert h.inline
    assert not h.attachment
    assert h.filename == 'foo.html'


def test_inlwithfnattach(content_disposition):
    h = content_disposition(b'Content-Disposition: inline; filename="Not an attachment!"')
    assert h.inline
    assert not h.attachment
    assert h.filename == 'Not an attachment!'


def test_inlwithasciifilenamepdf(content_disposition):
    h = content_disposition(b'Content-Disposition: inline; filename="foo.pdf"')
    assert h.inline
    assert not h.attachment
...
def test_attwithasciifilename25(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="0000000000111111111122222"')
    assert h.attachment
    assert not h.inline
    assert h.filename == '0000000000111111111122222'


def test_attwithasciifilename35(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="00000000001111111111222222222233333"')
    assert h.attachment
    assert not h.inline
    assert h.filename == '00000000001111111111222222222233333'


def test_attwithquotedsemicolon(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="Here\'s a semicolon;.html"')
    assert h.attachment
    assert h.filename == "Here's a semicolon;.html"


def test_attwithasciifnescapedchar(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="f\\oo.html"')
    assert h.attachment
    assert not h.inline
    assert h.filename == 'foo.html'
...
def test_attwithasciifnescapedquote(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="\\"quoting\\" tested.html"')
    assert h.attachment
    assert h.filename == '"quoting" tested.html'
...
def test_attwithfilenameandextparamescaped(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; foo="\\"\\\\";filename="foo.html"')
    assert h.attachment
    assert h.filename == 'foo.html'
    assert h.params['foo'] == '"\\'
...
def test_attwithisofnplain(content_disposition):
    ...
    assert h.attachment
    assert h.filename == 'foo-\xe4.html'
...
def test_attwithutf8fnplain(content_disposition):
    ...
    assert h.attachment
    assert h.filename == 'foo-\xc3\xa4.html'
...
def test_attwithnamepct(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; name="foo-%41.html"')
    assert h.attachment
    assert not h.filename
    assert h.params['name'] == 'foo-%41.html'


def test_attwithfilenamepctandiso(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="\xe4-%41.html"')
    assert h.attachment
    assert h.filename == '\xe4-%41.html'
...
def test_attwithfnrawpctenclong(content_disposition):
    ...
    assert h.attachment
    assert h.filename == 'foo-%c3%a4-%e2%82%ac.html'
...
def test_attconfusedparam(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; xfilename=foo.html')
    assert h.attachment
    assert not h.filename
...
def test_attcdate(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; creation-date="Wed, 12 Feb 1997 16:29:51 -0500"')

    assert h.creation_date == datetime.datetime(1997, 2, 12, 16, 29, 51, tzinfo=datetime.timezone.utc)
    # assert h.creation_date == 855761391.0  # FIXME: assert <HTTP Date(855764991)> == 855761391.0


def test_attmdate(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; modification-date="Wed, 12 Feb 1997 16:29:51 -0500"')
    assert h.modification_date == datetime.datetime(1997, 2, 12, 16, 29, 51, tzinfo=datetime.timezone.utc)
...
def test_dispext(content_disposition):
    h = content_disposition(b'Content-Disposition: foobar')
    assert h.attachment
    assert not h.params


def test_dispextbadfn(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; example="filename=example.txt"')
    assert h.attachment
    assert not h.filename
    assert h.params['example'] == 'filename=example.txt'
...
def test_attwithfn2231quot(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*="UTF-8\'\'foo-%c3%a4.html"')
    assert h.params['filename*'] == "UTF-8''foo-%c3%a4.html"


def test_attwithfn2231quot2(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*="foo%20bar.html"')
    assert h.params['filename*'] == 'foo%20bar.html'
...
def test_attwithfn2231nbadpct1(content_disposition):
    ...
    assert h.params['filename*'] == "UTF-8''foo%"
...
def test_attwithfn2231nbadpct2(content_disposition):
    ...
    assert h.params['filename*'] == "UTF-8''f%oo.html"
...
def test_attfncont(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*0="foo."; filename*1="html"')
    assert h.attachment
    assert h.filename == 'foo.html'


def test_attfncontqs(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*0="foo"; filename*1="\\b\\a\\r.html"')
    assert h.filename == 'foobar.html'


def test_attfncontenc(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*0*=UTF-8\'\'foo-%c3%a4; filename*1=".html"')
    assert h.attachment
    assert h.filename == 'foo-\xe4.html'


def test_attfncontlz(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*0="foo"; filename*01="bar"')
    assert h.attachment
    assert h.filename == 'foo'
    assert h.params['filename*01'] == 'bar'


def test_attfncontnc(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*0="foo"; filename*2="bar"')
    assert h.filename == 'foo'
    assert h.params['filename*2'] == 'bar'


def test_attfnconts1(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*1="foo."; filename*2="html"')
    assert h.attachment
    assert h.filename is None
    assert h.params['filename*1'] == 'foo.'
    assert h.params['filename*2'] == 'html'


def test_attfncontord(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*1="bar"; filename*0="foo"')
    assert h.attachment
    assert h.filename == 'foobar'


def test_attfnboth(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename="foo-ae.html"; filename*=UTF-8\'\'foo-%c3%a4.html')
    assert h.attachment
    assert h.filename == 'foo-\xe4.html'
...
def test_attfnboth2(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; filename*=UTF-8\'\'foo-%c3%a4.html; filename="foo-ae.html"')
    assert h.attachment
    assert h.filename == 'foo-\xe4.html'
...
def test_attfnboth3(content_disposition):
    ...
    assert h.params['filename*0*'] == "ISO-8859-15''euro-sign%3d%a4"


def test_attnewandfn(content_disposition):
    h = content_disposition(b'Content-Disposition: attachment; foobar=x; filename="foo.html"')
    assert h.attachment
    assert h.filename == 'foo.html'
...
def test_attrfc2047quoted(content_disposition):
    ...
    assert h.attachment
    assert h.filename == '=?ISO-8859-1?Q?foo-=E4.html?='
